...
class FeedVC: UIViewController {
    ...
    func setupTableView() {
        /* PART 2A START */
        var snapsTableView = UITableView(frame: CGRect(x: 0, y: 50, width: view.frame.width, height: view.frame.height - 20))
        snapsTableView.register(FeedTableViewCell.self, forCellReuseIdentifier: "feedCell")
        snapsTableView.delegate = self
        snapsTableView.dataSource = self
        
        view.addSubview(snapsTableView)
